...
## Configuration
- **Hostname**: `esp32-cam-4wd.local`
- **Password**: `robot123` (change in code if needed)
...
```ini
upload_port = 192.168.220.232  ; Replace with your ESP32's IP
```
...
1. Open Arduino IDE
2. Go to Tools → Port
3. You should see: `esp32-cam-4wd at 192.168.220.232` (Network Port)
...
- Password in platformio.ini must match password in code
- Current password: `robot123`
- Change in both places if needed:
  - Code: `ArduinoOTA.setPassword("robot123");`
  - platformio.ini: `--auth=robot123`
